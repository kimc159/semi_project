...
// css, js, img 등이 저장되어 있는 폴더의 URL
// --> 템플릿에서 리소스를 참조할 때 사용한다.
$assets_url = $home_url . '/_template/assets';

// 첨부파일이 저장될 폴더
$upload_dir = '/_files';    // -->  상대경로
$upload_abs_path = $document_root . $upload_dir;  // --> 서버상의 절대 경로
$upload_url = $home_url . $upload_dir;            // --> 웹 상의 절대 경로

/* 템플릿 설정하기 */
$tpl = new Template_();

// 템플릿 파일들이 저장될 경로를 사이트 전역에 걸쳐서 일괄 설정하기
...
if ($ses_member_id > 0) {
    ...
    if ($result === false) {
        redirect(false, '회원정보 조회에 실패했습니다. 관리자에게 문의 바랍니다.');
    }
