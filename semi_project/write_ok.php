...
<?php
    ...
    if($member_info !== false){
    	// 로그인한 경우
    	$sql = "INSERT INTO bbs_document(bbs_type, member_id, writer_name, writer_pw, email, subject, content, hit, reg_date, edit_date) VALUES ('%s', %d, '%s', '%s', '%s', '%s', '%s', 0, now(), now())";
    	$input = array($bbs_type, $member_id, $writer_name, $writer_pw, $email, $subject, $content);
    }else{
    	//로그인 하지 않은 경우
    	$sql = "INSERT INTO bbs_document (bbs_type, member_id, writer_name, writer_pw,email, subject, content, hit, reg_date, edit_date) VALUES ('%s', NULL, '%s', password('%s'),'%s', '%s', '%s', 0, now(), now())";
    ...
    if (is_array($file)) {
        ...
        for ($i = 0; $i < count($file); ++$i) {
            $sql = "INSERT INTO bbs_file (bbs_document_id, file_name, file_type,file_size, file_path, reg_date, edit_date) VALUES (%d, '%s', '%s', %d, '%s', now(), now())";
